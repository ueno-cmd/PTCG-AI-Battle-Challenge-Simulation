...
class TestScoreCardOption:
    ...
    def test_to_active_prefers_higher_hp_among_non_grimmsnarl_attackers(self):
        ...
        high_hp_yveltal = make_pokemon(id=gm.Yveltal, hp=110)
        my_ps = make_player_state(active_pokemon=make_pokemon(id=1), bench=[low_hp_impidimp, high_hp_yveltal])
        op_ps = make_player_state(active_pokemon=make_pokemon(id=2, hp=200))
        obs = self._make_obs(my_ps, op_ps)
        fs = self._make_fs()
        o_low  = Option(type=OptionType.CARD, area=AreaType.BENCH, index=0, playerIndex=0)
        o_high = Option(type=OptionType.CARD, area=AreaType.BENCH, index=1, playerIndex=0)
        score_low  = gm._score_card_option(obs, o_low, SelectContext.TO_ACTIVE, 0, fs, defaultdict(int))
        score_high = gm._score_card_option(obs, o_high, SelectContext.TO_ACTIVE, 0, fs, defaultdict(int))
