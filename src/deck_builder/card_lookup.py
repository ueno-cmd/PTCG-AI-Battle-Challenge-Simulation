...
def find_card_id(name: str, card_dict: dict[str, int]) -> tuple[int | None, list[str], bool]:
    """
    カード名からIDを検索する。

    Returns:
        (card_id, candidates, fuzzy_match)
        - card_id: 一致したID。見つからなければ None
        - candidates: 部分一致候補のカード名リスト（card_id が None のときのみ意味を持つ）
        - fuzzy_match: True の場合は正規化後に一致（大文字小文字・スペース差異あり）
    """
    # 完全一致
    if name in card_dict:
        return card_dict[name], [], False
